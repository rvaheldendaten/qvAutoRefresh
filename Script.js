...
Qva.AddDocumentExtension('AutoRefresh', function () {
	...
	var cRefreshEnabled_VariableName = 'vAutoRefresh_Enabled';
	...
	_this.ExtSettings.RefreshEnabled = 0; // Default Refresh not enabled
    //ConsoleDebug(_this);


    function Init() {
        ConsoleLog('Initialize AutoRefresh extension ...', enableConsoleOutput);
        ...
        RetrieveRefreshInterval(function () {
			...
			if (_this.ExtSettings.RefreshEnabled == 0 ) 
			{
				ConsoleLog('AutoRefresh disabled!', enableConsoleOutput); 
			}
			else
			{
            ...
            ConsoleLog('Finished loading AutoRefresh extension ...', enableConsoleOutput);
                ...
                if ((obj.isreserved == "false") && (obj.isconfig == "false")) {
                    ...
                    if (obj.name.toLowerCase() == cRefreshInterval_VariableName.toLowerCase()) {
                        try {
                            ...
                            ConsoleLog('Set Refresh Interval value to ' + obj.value.toString(), enableConsoleOutput);
                        } catch (e) {
                            ConsoleError('Value of cRefreshInterval_VariableName is not a valid int value', enableConsoleOutput);
                        }
                       
                    }
					if (obj.name.toLowerCase() == cRefreshEnabled_VariableName.toLowerCase()) {
                        try {
                            _this.ExtSettings.RefreshEnabled = parseInt(obj.value);
                            ConsoleLog('Set RefreshEnabled to ' + obj.value.toString(), enableConsoleOutput);
                        } catch (e) {
                            ConsoleError('Value of RefreshEnabled is not a valid int value', enableConsoleOutput);
                        }
                        ;
                    }
                }
